...
public class KeyCloakAdminService {
    ...
    @Value("${keycloak.admin.server_url}")
    ...
    @Value("${keycloak.admin.client_uuid}")
    private String clientUUid;

    private final RestTemplate restTemplate = new RestTemplate();

    // To interact with all this endpoint we are using KeyCloak Admin Rest API
    ...
    public String createUser(String token, UserRequest userRequest){
        ...
        if(!HttpStatus.CREATED.equals(response.getStatusCode())){
            throw new RuntimeException("Failed to creat User inside Keycloak" + response.getBody());
        }

        URI location = response.getHeaders().getLocation();
        if(location == null){
            throw new RuntimeException("Failed to creat User inside Keycloak as KeyCloak didn't return Location header" + response.getBody());
        }
